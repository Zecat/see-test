...
class TestCallExpression:
    ...
    def to_expectation(self, node):
        ...
        assert_expect_str = self.get_eq_assertion_str(input_var_name, expect_var_name)
    ...
    def add_call_expression_from_node(self, node):
        ...
        # TODO clean up the capture output functionality
        expect_output_text = None

        if expect_output_node:
            expect_output_text = self.get_node_text(expect_output_node)
        ...
        if expect_return_node:
            ...
            fn_assertion_str = self.get_eq_assertion_str(fcall_str, expect_return_text)
            self.body.append(fn_assertion_str)
        else:
            self.body.append(fcall_str + ";")

        if expect_output_text != None:
            ouput_assertion_str = self.get_eq_assertion_str(
                output_var, expect_output_text
            )
            self.body.append("}\n\n" + ouput_assertion_str)

        self.body += expect_assertions

    def get_eq_assertion_str(self, a, b):
